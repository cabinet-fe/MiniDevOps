...
func (pq *ProjectQuery) QueryBuilds() *BuildTaskQuery {
	query := (&BuildTaskClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, project.BuildsTable, project.BuildsColumn),
		)
		fromU = sqlgraph.SetNeighbors(pq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (pq *ProjectQuery) Clone() *ProjectQuery {
	...
	return &ProjectQuery{
		...
		withBuilds: pq.withBuilds.Clone(),
...
func (pq *ProjectQuery) WithBuilds(opts ...func(*BuildTaskQuery)) *ProjectQuery {
	query := (&BuildTaskClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := pq.withBuilds; query != nil {
		if err := pq.loadBuilds(ctx, query, nodes,
			func(n *Project) { n.Edges.Builds = []*BuildTask{} },
			func(n *Project, e *BuildTask) { n.Edges.Builds = append(n.Edges.Builds, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProjectQuery) loadBuilds(ctx context.Context, query *BuildTaskQuery, nodes []*Project, init func(*Project), assign func(*Project, *BuildTask)) error {
	...
	query.Where(predicate.BuildTask(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.BuildsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.project_builds
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_builds" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_builds" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
