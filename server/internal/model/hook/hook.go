...
type BuildTaskFunc func(context.Context, *model.BuildTaskMutation) (model.Value, error)

// Mutate calls f(ctx, m).
func (f BuildTaskFunc) Mutate(ctx context.Context, m model.Mutation) (model.Value, error) {
	if mv, ok := m.(*model.BuildTaskMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *model.BuildTaskMutation", m)
}
...
func (f UserFunc) Mutate(ctx context.Context, m model.Mutation) (model.Value, error) {
	if mv, ok := m.(*model.UserMutation); ok {
		return f(ctx, mv)
	}
