...
function connectWS() {
  ...
  if (!token || !runId.value) return;
  const url = buildRunLogsWSURL(runId.value, token);
...
async function hydrateLogHTTP() {
  try {
    const text = await getBuildRunLog(runId.value);
    if (text && logLines.value.length === 0) {
...
async function onRedeploy() {
  ...
  try {
    ...
    logLines.value.push("=== Redeploy requested ===");
    connectWS();
    startPolling();
  } catch (err) {
