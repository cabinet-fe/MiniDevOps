...
| **超级管理员（Super Admin）**   | 内置最高权限账号/角色，可访问运维模块                                                                      | 普通自定义角色                        |
...
| 概念           | 说明                                                                                                   |
| -------------- | ------------------------------------------------------------------------------------------------------ |
| 用户           | 登录主体；可禁用；拥有一个或多个角色（至少实现「主角色」或角色集合，产品验收以「可配置角色授权」为准） |
| 内置超级管理员 | 系统初始化创建；不可删除；始终拥有全部权限；是运维模块唯一准入身份类型                                 |
| 自定义角色     | 管理员可创建/编辑/删除；绑定权限资源集合                                                               |
...
其中 `path` 为 **一层或多层** 资源路径，层与层之间用 `.` 连接：

```text
level1:action
level1.level2:action
level1.level2.level3:action
…
```

| 段                      | 说明                                                                                                      |
| ----------------------- | --------------------------------------------------------------------------------------------------------- |
| `level1` / `level2` / … | 资源路径层级；语义由资源定义决定（菜单分组、页面、卡片、按钮所属对象等），**不**固定解释为「模块 + 页面」 |
| `action`                | 动作类型；与 path 用 **唯一一个 `:`** 分隔                                                                |

**为何不用 `module.page`：** 并非所有资源都有「页面」嵌套。例如仪表盘本身可以是一级资源 `dashboard:view`，其下卡片才是 `dashboard.system_info:view`；运维分组可以是 `ops:view`，子页为 `ops.processes:view`。用「层级 path」表达，避免把没有页面嵌套的资源硬套成 module.page。

**约定：**

1. **`.`** 只分隔 path 层级，不表示动作。
2. **`:`** 在整串中只出现一次，其后为 `action`。
3. 常用动作：`view`、`create`、`update`、`delete`、`execute`；可按资源扩展（如 `download`、`cancel`、`retry`、`redeploy`、`install`、`test`、`use`）。
4. 进入某菜单/页面至少需要对应 path 的 `:view`；按钮与 API 校验更细动作。
5. 角色授权 UI 按资源树（含菜单树）勾选，结果存为完整 `{path}:action` 字符串集合。

**示例：**

| 权限码                         | 含义                                        |
| ------------------------------ | ------------------------------------------- |
| `dashboard:view`               | 查看/进入仪表盘（一级菜单）                 |
...
| `project.docs:execute`         | 触发智能体生成文档                          |
| `ai.agents:execute`            | 手动触发智能体                              |
...
权限资源目录**必须包含左侧菜单**，菜单是一等资源，而非前端写死的导航表。
...
| route        | 前端路由（叶子菜单）                                                                                                                                                                                                 |
| sort         | 排序                                                                                                                                                                                                                 |
| icon         | **仅一级菜单：支持上传图标**；前端将图片转为 **Base64** 后持久化存储（可存 data URL 或纯 Base64 字符串）。**原始图片体积不得超过 32KB**（超限拒绝保存），避免登录/菜单接口体积膨胀。预置菜单可带默认图标，仍允许替换 |
| enabled      | 是否启用                                                                                                                                                                                                             |

**规则：**

1. **所有一级菜单**均支持上传/更换图标；图标以 **Base64 内嵌**方式随菜单数据存储与下发，**不**走独立静态文件 URL。上传前校验：**图片原始大小 ≤ 32KB**，否则返回明确错误（如 400）。建议前端压缩/裁剪至合适尺寸后再转 Base64。
2. 用户**登录成功**（及 `GET /auth/me` 刷新身份）时，服务端根据其角色已授权的菜单资源，返回**裁剪后的菜单树**（含一级图标 Base64、标题、路由、排序、子菜单）。
3. 前端侧栏**只渲染**接口返回的菜单，不本地硬编码全量菜单后再隐藏。
4. 无某菜单 `:view`（或角色未绑定该菜单资源）→ 不出现在返回树中；对应路由直达仍由服务端 403 兜底。
5. 一级菜单若仅作分组：当且仅当至少有一个可见子菜单时才出现在树中（或显式授予 `level1:view` 且存在可见子项，产品实现二选一，需行为一致）。
6. 二级及以下菜单图标可选；若支持，同样遵守 Base64 存储与 **≤ 32KB** 限制。

#### 4.2.3 通用规则

- 无对应资源权限 → 菜单不下发、路由不可进入、API 返回 403、仪表盘相关卡片不可配置且不可显示。
- 运维相关 path（如 `ops` 及其子级）仅对超级管理员开放；即使自定义角色误勾选，服务端必须拒绝。
- 产品项目内权限与全局 RBAC **同时生效**：全局无对应 `project…:view`（或更细权限）则不可进入；进入后还受项目成员角色约束。
...
左侧导航**来源于菜单类权限资源**，登录后按角色下发，不是前端静态写死后再按角色隐藏。
...
1. `POST /auth/login` 与 `GET /auth/me` 的响应中包含：用户信息、权限码列表（或等价结构）、**菜单树**。
2. 菜单树节点至少含：`path`、`title`、`route`、`sort`、`icon`（一级为 Base64 图标数据）、`children`。
...
| 卡片         | 内容                                                      | 绑定权限（示例）               |
| ------------ | --------------------------------------------------------- | ------------------------------ |
| 构建任务卡片 | 运行中/排队数量、最近执行结果、成功率摘要、跳转到执行详情 | `cicd.build_runs:view`         |
| 系统信息卡片 | 产品版本、OS、架构、Go/运行时信息、启动时间、主机名等     | `dashboard.system_info:view`   |
| 系统状态卡片 | CPU/内存/磁盘占用、服务健康（API 存活）、关键目录可用空间 | `dashboard.system_status:view` |

> 说明：进入仪表盘需 `dashboard:view`；卡片另受上表 path 的 `:view` 约束。系统信息/状态对非超管可只读展示「安全摘要」；进程明细与危险操作仍仅在运维模块。
...
1. 无 `cicd.build_runs:view` 的角色看不到构建任务卡片且无法添加。
2. 用户调整顺序后刷新仍保持。
...
- 仅超级管理员可见菜单、可调用 API。
- 系统支持多用户，但运维域与业务 RBAC 正交：**非超管永远不可进入**。
...
- 创建项目者默认为负责人。
- 非项目成员即使有全局 `project.projects:view`，也只能看到被授权可见的项目列表策略（建议：有全局管理权限者可见全部；普通用户仅见自己加入的项目）。**待确认项见 §19**——首期采用：**具备全局项目管理类权限（如 `project.projects:update`）可看全部；否则仅看自己加入的项目**。
...
| 实体                                               | 类型      | 说明                                                                  |
| -------------------------------------------------- | --------- | --------------------------------------------------------------------- |
| User / Role / RolePermission / RbacResource / Menu | 配置      | 身份与授权；资源 code 为 `{path}:action`；Menu.icon 为 Base64，≤ 32KB |
